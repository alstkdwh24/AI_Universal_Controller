...
export default function LoginView({ onClose }) {
    ...
    const handleSocialLogin = (url) => {
        // Electron 환경: ?client=electron 파라미터 추가 후 외부 브라우저로 열기
        // → 백엔드 ClientTypeFilter가 감지 → 세션 저장
        // → SuccessHandler에서 jo-gpt://auth?token=XXX 딥링크로 리다이렉트
        if (window.electronAPI && window.electronAPI.openExternal) {
            window.electronAPI.openExternal(url + "?client=electron");
        } else {
            // 웹 브라우저 환경 fallback (파라미터 없이 그냥 이동)
